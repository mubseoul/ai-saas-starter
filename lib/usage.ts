...
> {
  ...
  return usageRecords.map((record: { month: number; year: number; requestCount: number }) => ({
    month: record.month,
    year: record.year,
...
export async function getUsageStats() {
  ...
  return {
    ...
    totalRequests,
    topUsers: topUsers.map((u: { user: { name: string | null; email: string }; requestCount: number }) => ({
